...
void printWordGraph(word_t*,int,size_t*,double*);
void printLetterGraph(char_t*,int,size_t*,double*);
...
void printWordGraph(word_t* maxArray,int length,size_t* count,double* countAll){
	double frequency;
	if(*count<length){
		length=*count;
	}
	unsigned long int maxWordLength=0;
	for(int i=0;i<length;i++){
...
void printLetterGraph(char_t* maxArray,int length,size_t* count,double* countAll){
	if(*count<length){
		length=*count;
	}
	for(int i=0;i<length;i++){
